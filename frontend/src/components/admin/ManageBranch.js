...
useEffect(() => {
    ApiServices.getBranch({}).then((response) => {
        console.log("Branch Get All API Response:", response);    
  ...
  const changeStatus=(id,status)=>{
    ...
    ApiServices.softdeleteBranch(qs.stringify(data)).then((res)=>{
    ...
    <div className="row">
      ...
      >
        <div className="text-center wow fadeInUp" data-wow-delay="0.1s">
            <h3 className="mb-3 bg-white text-center px-3">Manage Branch</h3>
            </div>
        <table className="table table-bordered wow fadeInUp" data-wow-delay="0.1s">
          ...
          {data?.map((el,index)=>(
            <tr key={index}>
              ...
              <td>
                <Link to={"/admin/editbranch/"+el._id} className="text-primary">
