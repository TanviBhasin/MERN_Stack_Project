...
useEffect(() => {
    ApiServices.getCourse({}).then((response) => {
        console.log("Courses Get All API Response:", response);    
  ...
  const changeStatus=(id,status)=>{
    ...
    ApiServices.softdeleteCourse(qs.stringify(data)).then((res)=>{
    ...
    <div className="row">
      ...
      >
        <div className="text-center wow fadeInUp" data-wow-delay="0.1s">
            <h3 className="mb-3 bg-white text-center px-3">Manage Courses</h3>
            </div>
          ...
          {data?.map((el,index)=>(
            <tr key={index}>
              ...
              <td>
                ...
                <Link to={"/admin/editcourses/"+el._id} className="text-primary">
