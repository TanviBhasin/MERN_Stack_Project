...
export default function ManageAssignSub() {
  const [data,setData]=useState([])
  const [isChange,setIsChange]=useState(false)
  
  useEffect(() => {
      ApiServices.getAssignSubject({}).then((response) => {
          console.log("Assign Subject Get All API Response:", response);    
    ...
    const changeStatus=(id,status)=>{
      ...
      ApiServices.softdeleteAssignSubject(qs.stringify(data)).then((res)=>{
    ...
    <div className="row">
      ...
      >
        <div className="text-center wow fadeInUp" data-wow-delay="0.1s">
            <h3 className="mb-3 bg-white text-center px-3">Manage Assigned Subjects</h3>
            </div>
        <table className="table table-bordered wow fadeInUp" data-wow-delay="0.1s">
          ...
          {data?.map((el,index)=>(
            <tr key={index}>
              ...
              <td>
                <Link to={"/admin/editassignsub/"+el._id} className="text-primary">
